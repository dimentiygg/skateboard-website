  ...
  return (
    <footer className="bg-texture bg-zinc-900 text-white overflow-hidden">
      <div className="relative h-[75vh] ~p-10/16 md:aspect-auto">
        ...
        />
        <Logo className="pointer-events-none relative h-20 mix-blend-exclusion md:h28" />
      </div>
      <Bounded as="nav">
        <ul className="flex flex-wrap justify-center gap-8 ~text-lg/xl">
          {settings.data.navigation.map(item => (
            <li key={item.link.text} className="hover:underline">
              {item.link.text === 'Team' ? (
                <ScrollLink to="team">{item.link.text}</ScrollLink>
              ) : item.link.text === 'About' ? (
                <ScrollLink to="about">{item.link.text}</ScrollLink>
              ) : (
